...
<html>
	...
	<body>
		<table border="0">
			<thead>
				<th colspan="5">largest number of votes in blantyre city central</th>
			</thead>
			<tbody>
				<?php
						...
						echo "<td>
							  <table border='1'>";
								...
								echo "</tr>";
								$query = ("
#councilor list from blantyre city central
...
union
#president winner for blantyre city central
		...
		position_name='President' AND vote.polling_centre_name IN(SELECT polling_centre_name FROM polling_centre
			WHERE ward_name IN (SELECT ward_name FROM ward WHERE constituency_name IN (SELECT constituency_name
				FROM constituency WHERE district_code IN(SELECT district_code FROM district WHERE 
					region_name='southern')))) AND vote.polling_centre_name IN(SELECT polling_centre_name FROM polling_centre
			WHERE ward_name IN (SELECT ward_name FROM ward WHERE constituency_name = ('Blantyre-city-central')))) AS votes FROM candidate LEFT JOIN citizen
...
party_code, represent, (SELECT COUNT(candidate_id) AS vID
		...
		position_name='President' AND vote.polling_centre_name IN(SELECT polling_centre_name FROM polling_centre
			WHERE ward_name IN (SELECT ward_name FROM ward WHERE constituency_name IN (SELECT constituency_name
				FROM constituency WHERE district_code IN(SELECT district_code FROM district WHERE 
					region_name='southern')))) AND vote.polling_centre_name IN(SELECT polling_centre_name FROM polling_centre
			WHERE ward_name IN (SELECT ward_name FROM ward WHERE constituency_name = ('Blantyre-city-central')))
