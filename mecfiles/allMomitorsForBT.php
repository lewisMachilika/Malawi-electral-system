...
#back{
	...
	<body>
		<table border="0">
			<thead>
				<th colspan="5">all monitors for blantyre</th>
			</thead>
			<tbody>
				<?php
...
WHERE citizen.citizen_id= monitor.citizen_id AND citizen.citizen_id IN (SELECT citizen_id 
 FROM monitor WHERE  polling_centre_name IN(SELECT polling_centre_name FROM
	polling_centre WHERE ward_name IN(SELECT ward_name FROM ward WHERE 
		constituency_name IN(SELECT constituency_name FROM constituency WHERE district_code='BT'))))"
                ...
                <button><a href="allMomitorsForBT.php" class="btn btn-primary btn-lg btn-danger pull-right " >all</a></button>
                </div>
                <div class="form-group" id="maleMonitor">
                <button><a href="allMaleMonitorsForBT.php" class="btn btn-primary btn-lg btn-danger pull-right " >male monitors</a>
                </button></div>
                <div class="form-group" id="femaleMonitor">
                <button>
    	        <a href="allFemaleMonitorsForBT.php" class="btn btn-primary btn-lg btn-danger pull-right " >female monitors</a>
                </button></div>
