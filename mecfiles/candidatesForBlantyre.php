...
<html>
	...
	position_name AS position, represent, (SELECT COUNT(candidate_id)
		FROM vote WHERE vote.candidate_id=K.candidate_id AND polling_centre_name IN(SELECT polling_centre_name FROM
	polling_centre WHERE ward_name IN(SELECT ward_name FROM ward WHERE 
		constituency_name IN(SELECT constituency_name FROM constituency WHERE district_code ='BT'))) ) AS votes
	 FROM 
	citizen c, candidate k WHERE c.citizen_id = k.citizen_id AND ((represent IN
	( SELECT constituency_name FROM constituency WHERE 
		district_code ='BT')) OR represent IN
	(SELECT ward_name FROM ward WHERE constituency_name IN( SELECT constituency_name FROM constituency WHERE district_code='BT')) 
	OR represent=('Malawi'))
