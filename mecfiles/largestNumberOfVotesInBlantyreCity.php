...
union
#president list for blantyre city
...
party_code, represent, (SELECT COUNT(candidate_id) AS vID
		...
		position_name='President' AND vote.polling_centre_name IN(SELECT polling_centre_name FROM polling_centre
			...
			WHERE ward_name IN (SELECT ward_name FROM ward WHERE constituency_name IN ('Blantyre-City-central',
				'Blantyre-City-east')))) AS votes FROM candidate LEFT JOIN citizen
